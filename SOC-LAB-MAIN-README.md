...
└─────────────────────────────────────────────────────────────────────┘
        ...
        │  - AD Domain: dc-01.lab │   │  │  192.168.X.80   │    │
        │  - Arc Agent            │   │  │  (Indexer)      │    │
...
|---|---|---|---|:---:|
| **KALI01** | 192.168.X.10 | Kali Linux | Attack workstation | ✅ Active |
| **DC01** | 192.168.X.20 | Windows Server 2022 | Domain Controller | ✅ Active |
| **WIN-01** | 192.168.X.30 | Windows 10 | Domain workstation | 🔜 Planned |
| **SPLUNK-01** | 192.168.X.80 | Ubuntu Server 22.04 | Splunk Enterprise indexer | ✅ Active |

> **Network Configuration Note:** Replace `X` with your chosen subnet number (e.g., `192.168.100.0/24`). This lab uses `192.168.70.0/24` as an example. Adjust based on your environment to avoid IP conflicts with other lab networks.

### Infrastructure

- **Hypervisor:** VMware Workstation Pro / ESXi
- **Network:** VMware host-only network on 192.168.X.0/24 (isolated lab)
- **Internet:** Separate VMware NAT interface for outbound access (optional)
...
```bash
# Attack (from KALI01; substitute X with your subnet, e.g., 192.168.100.20)
nxc smb 192.168.X.20 -u Administrator -p 'WrongPassword'
...
From KALI01 (substitute X with your subnet):
```bash
nxc smb 192.168.X.20 -u Administrator -p 'WrongPassword'
# Example: nxc smb 192.168.100.20 -u Administrator -p 'WrongPassword'
```
