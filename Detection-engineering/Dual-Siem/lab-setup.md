...
> **IP Address Format:** Throughout this guide, `192.168.X.x` represents your lab subnet. Substitute `X` with your chosen subnet number (e.g., `192.168.100.x`). This lab uses `192.168.70.x` as an example.
...
From Splunk Web UI (http://192.168.X.80:8000, e.g., 192.168.100.80:8000):
...
# Install
Start-Process -FilePath msiexec.exe -ArgumentList "/i $env:TEMP\splunk-uf.msi /quiet RECEIVING_INDEXER=`"192.168.X.80:1137`" WINEVENTLOG_SEC_MONITOR=1 WINEVENTLOG_SYS_MONITOR=1 WINEVENTLOG_APP_MONITOR=1" -Wait
...
[tcpout:lab_indexers]
server = 192.168.X.80:1137
# Replace X with your subnet number, e.g., 192.168.100.80:1137
...
```powershell
# On DC01, check connectivity (substitute X with your subnet)
...
```bash
# From KALI01 (substitute X with your subnet, e.g., 192.168.100.20)
nxc smb 192.168.X.20 -u Administrator -p 'WrongPassword' 2>&1 | head -5
```
